            ...
            <?php else: ?>
                ...
                <div class="project-card" onclick="goToProjectLedger(<?= $project['id'] ?>)">
                    ...
                    <div class="project-actions">
                        ...
                        <button class="btn btn-sm" style="background: linear-gradient(135deg, #8b5cf6 0%, #6366f1 100%); color: white;" onclick="event.stopPropagation(); openDivisionBreakdownModal(<?= $project['id'] ?>, '<?= htmlspecialchars($project['project_name'], ENT_QUOTES) ?>')">
                            📈 Breakdown
                        </button>
...
<!-- Modal: Division Expense Breakdown -->
<div id="divisionBreakdownModal" class="modal">
    <div class="modal-content" style="max-width: 600px;">
        <div class="modal-header">
            <h3 id="divisionModalTitle" style="margin: 0; font-size: 1.25rem; font-weight: 600;">Division Expense Breakdown</h3>
            <span class="close" onclick="closeModal('divisionBreakdownModal')">&times;</span>
        </div>
        <div class="modal-body">
            <div style="margin-bottom: 1.5rem; padding: 1rem; background: linear-gradient(135deg, #667eea15 0%, #764ba215 100%); border-radius: 12px; border: 1px solid #e5e7eb;">
                <div style="text-align: center;">
                    <div style="font-size: 0.85rem; color: #6b7280; margin-bottom: 0.5rem;">Total Pengeluaran Per Divisi</div>
                    <div id="divisionTotalAmount" style="font-size: 1.75rem; font-weight: 700; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;">Rp 0</div>
                </div>
            </div>
            ...
            <div id="divisionLegend" style="display: grid; gap: 0.75rem;">
                <!-- Legend items will be populated by JavaScript -->
            </div>
            
            <div id="divisionEmptyState" style="display: none; text-align: center; padding: 2rem; color: #9ca3af;">
                ...
                <p style="margin: 0; font-size: 0.95rem;">Tidak ada data pengeluaran per divisi untuk project ini.</p>
            </div>
        </div>
        <div class="modal-footer">
            <button type="button" class="btn btn-secondary" onclick="closeModal('divisionBreakdownModal')">Close</button>
        </div>
    </div>
</div>
...
async function openDivisionBreakdownModal(projectId, projectName) {
    try {
        // Update modal title
        document.getElementById('divisionModalTitle').textContent = `${projectName} - Breakdown by Division`;
        
        // Show modal
        document.getElementById('divisionBreakdownModal').style.display = 'block';
        
        // Show loading state
        document.getElementById('divisionLegend').innerHTML = '<div style="text-align: center; padding: 1rem; color: #9ca3af;">Loading...</div>';
        document.getElementById('divisionEmptyState').style.display = 'none';
        
        // Fetch division breakdown data
        const response = await fetch(`<?= BASE_URL ?>/api/investor-division-breakdown.php?project_id=${projectId}`);
        const result = await response.json();
        
        if (!result.success) {
            throw new Error(result.message || 'Failed to fetch division data');
        }
        ...
        if (!data.divisions || data.divisions.length === 0) {
            document.getElementById('divisionEmptyState').style.display = 'block';
            document.getElementById('divisionLegend').innerHTML = '';
            document.getElementById('divisionTotalAmount').textContent = 'Rp 0';
            
            // Destroy existing chart
            if (divisionChartInstance) {
                divisionChartInstance.destroy();
                divisionChartInstance = null;
            }
            return;
        }
        
        // Update total amount
        document.getElementById('divisionTotalAmount').textContent = data.formatted_total;
        
        // Prepare chart data
        const labels = data.divisions.map(d => d.division_name);
        const amounts = data.divisions.map(d => d.amount);
        const colors = generateColors(data.divisions.length);
        ...
        // Create new chart
        const ctx = document.getElementById('divisionExpenseChart').getContext('2d');
        ...
        // Populate legend
        const legendHTML = data.divisions.map((division, index) => `
            <div style="display: flex; align-items: center; justify-content: space-between; padding: 0.75rem; background: linear-gradient(135deg, ${colors[index]}15 0%, ${colors[index]}08 100%); border-radius: 8px; border-left: 3px solid ${colors[index]};">
                <div style="display: flex; align-items: center; gap: 0.75rem; flex: 1;">
                    <div style="width: 12px; height: 12px; border-radius: 3px; background: ${colors[index]}; flex-shrink: 0;"></div>
                    <span style="font-size: 0.9rem; font-weight: 500; color: #374151;">${division.division_name}</span>
                </div>
                <div style="text-align: right;">
                    <div style="font-size: 0.95rem; font-weight: 600; color: #1f2937;">${division.formatted_amount}</div>
                    <div style="font-size: 0.75rem; color: #6b7280; margin-top: 2px;">${division.percentage}%</div>
                </div>
            </div>
        `).join('');
        
        document.getElementById('divisionLegend').innerHTML = legendHTML;
        
    } catch (error) {
        console.error('Error loading division breakdown:', error);
        alert('❌ Error loading division breakdown: ' + error.message);
        closeModal('divisionBreakdownModal');
    }
}
