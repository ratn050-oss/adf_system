<?php
// One-time deploy helper — triggers file sync when called with secret token
// REMOVE THIS BLOCK after deployment is confirmed working
if (isset($_GET['_deploy']) && hash_equals('adf2025sync', $_GET['_deploy'] ?? '')) {
    header('Content-Type: text/plain; charset=utf-8');
    $repo = 'ratn050-oss/adf_system';
    $branch = 'main';
    $webBase = '/home/adfb2574/public_html/narayanakarimunjawa.com';
    $deployFiles = [
        'website/public/index.php' => [$webBase . '/public/index.php', $webBase . '/index.php'],
        'website/public/activities.php' => [$webBase . '/public/activities.php', $webBase . '/activities.php'],
    ];
    $ctx = stream_context_create(['http' => ['timeout' => 30, 'user_agent' => 'ADF-Deploy/1.0']]);
    foreach ($deployFiles as $src => $dests) {
        $url = "https://raw.githubusercontent.com/$repo/$branch/$src";
        $content = @file_get_contents($url, false, $ctx);
        if ($content === false) { echo "FAIL download: $src\n"; continue; }
        foreach ($dests as $dest) {
            $dir = dirname($dest);
            if (!is_dir($dir)) @mkdir($dir, 0755, true);
            echo (@file_put_contents($dest, $content) !== false)
                ? "OK: $dest (" . strlen($content) . " bytes)\n"
                : "FAIL write: $dest\n";
        }
    }
    // Also update sync.php on ADF root
    $syncUrl = "https://raw.githubusercontent.com/$repo/$branch/sync.php";
    $syncContent = @file_get_contents($syncUrl, false, $ctx);
    if ($syncContent !== false) {
        $syncDest = '/home/adfb2574/public_html/sync.php';
        echo (@file_put_contents($syncDest, $syncContent) !== false)
            ? "OK: $syncDest (" . strlen($syncContent) . " bytes)\n"
            : "FAIL write: $syncDest\n";
    }
