<?php
/**
 * Setup Accounting Upgrade - CLI Version (Local Development)
 * Force local database configuration
 */

// Force local environment for CLI
define('APP_ACCESS', true);

// Force LOCAL database config for CLI
...
try {
    ...
    echo "✅ Connected to master database successfully!\n\n";

    $masterPdo->beginTransaction();

    // Get all active businesses
    echo "📍 Fetching active businesses...\n";
    ...
    foreach ($businesses as $biz) {
        ...
        try {
            ...
            echo "   📋 Adding column to cash_book...\n";
            try {
            ...
            if ($result['cnt'] == 0) {
                $bizDb->prepare("INSERT INTO cash_accounts (business_id, account_name, account_type, is_default_account, description, is_active) VALUES (?, ?, ?, 1, ?, 1)")->execute([$biz['id'], 'Kas Operasional', 'cash', 'Kas untuk pendapatan operasional']);
                $bizDb->prepare("INSERT INTO cash_accounts (business_id, account_name, account_type, description, is_active) VALUES (?, ?, ?, ?, 1)")->execute([$biz['id'], 'Kas Modal Owner', 'owner_capital', 'Dana dari pemilik untuk kebutuhan operasional harian']);
                $bizDb->prepare("INSERT INTO cash_accounts (business_id, account_name, account_type, description, is_active) VALUES (?, ?, ?, ?, 1)")->execute([$biz['id'], 'Bank', 'bank', 'Rekening bank utama bisnis']);
                echo "      ✅ 3 default accounts created\n";
            } else {
                echo "      ℹ️  Accounts exist (skip)\n";
            }
    ...
    echo "📊 Summary:\n";
    echo "   ✅ $successCount businesses processed successfully\n";
    if ($errorCount > 0) echo "   ⚠️  $errorCount businesses had errors\n";
    echo "   ✅ cash_accounts tables created\n";
    echo "   ✅ cash_account_transactions tables created\n";
    echo "   ✅ Default cash accounts initialized\n\n";

    echo "🎯 Default Cash Accounts Created:\n";
    echo "   1. Kas Operasional (pendapatan penjualan)\n";
    echo "   2. Kas Modal Owner (dana dari pemilik)\n";
    echo "   3. Bank (transaksi bank)\n\n";
